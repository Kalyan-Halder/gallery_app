  ...
  try {
    ...
    if(!user_id){
      ...
      if (!userExist) {
        ...
        saved_posts.sort(
          (a, b) => ids.indexOf(String(a._id)) - ids.indexOf(String(b._id))
        );
...
router.route("/delete_post").post(async (req, res) => {
  try {
    ...
    if (!post || post.user_id != userExist._id) {
      return res
        .status(400)
        .json({ message: "Post Does Not Exist or can not be deleted" });
    } else {
...
router.route("/save_post").post(async (req, res) => {
  try {
    ...
    if (!post_id || !token) {
      return res
        .status(400)
        .json({ message: "post_id and token are required" });
    }
    ...
    if (alreadySaved) {
      ...
      return res
        .status(201)
        .json({ message: "Post removed from saved", saved: false });
    } else {
      await user.updateOne(
        { _id: userExist._id },
      ...
      return res
        .status(200)
        .json({
          saved_posts: user_with_updated_saved_post.savedPost,
          message: "Post saved",
          saved: true,
        });
    }
  ...
  try {
    ...
    if (alreadyLiked) {
      ...
      await Promise.all([
        ...
        posts.updateOne({ _id: post_id }, { $inc: { likes: -1 } }),
      ]);
