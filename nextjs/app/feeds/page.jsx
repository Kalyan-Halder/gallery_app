    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.log("Fetched posts:", fetchedPosts[0].watcher_savedPosts);

          // Set posts directly from the API response
          setPosts(fetchedPosts);
          setSavedPosts_toggle(fetchedPosts[0].watcher_savedPosts);
          setLikedPosts_toggle(fetchedPosts[0].watcher_likedPosts);
          setLoading(false);
        }
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      const response = await fetch(`${BaseUrl}/like_post`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ post_id: postId, token: token }),
      });
        ...
        setPosts((prevPosts) =>
          prevPosts.map((post) => {
            if (String(post.id || post._id) === String(postId)) {
              return {
                ...post,
                likes: result.updatedLikes || post.likes,
              };
            }
            return post;
          })
  ...
  const handleSave = async (postId) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setSavedPosts_toggle(result.saved_posts);
  ...
  return (
    <div className="h-auto bg-gray-200">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 mb-6">
          <div className="flex justify-between px-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-linear-to-r from-blue-500 to-purple-500 text-white rounded-full hover:opacity-90 transition-opacity"
            >
              Post
            </button>
          </div>
        ...
        {toggle && (
          <Add_Post open={toggle} onClose={toggle_button} token={user_token} />
        )}
          ...
          {posts.length > 0 ? (
            posts.map((post) => (
              ...
              >
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-censter space-x-3">
                      <div
                        className={`h-10 w-10 rounded-full bg-linear-to-r ${
                          post.user?.color || "from-gray-500 to-gray-700"
                        } flex items-center justify-center text-white font-semibold bg-center bg-cover`}
                        style={
                          post.user?.avatarUrl
                      ...
                      >
                        {" "}
                        {!post.user?.avatarUrl && (post.user?.avatar || "UU")}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center space-x-2">
                      {post.location && (
                        <span className="text-sm text-gray-500 flex gap-2 justify-baseline align-middle">
                          {post.location} <MapMinus></MapMinus>
                        </span>
                      )}
                ...
                <div className="relative">
                  ...
                  >
                    <Bookmark
                      className={`h-7 w-7 ${
                        (savedPosts || []).some(
                          (id) => String(id) === String(post.id || post._id)
                        )
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-4">
                      <button
                        ...
                        className="flex items-center space-x-l hover:cursor-pointer"
                      >
                        <Heart
                          className={`h-6 w-6 ${
                            (likedPosts || []).some(
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                      <span
                        ...
                        style={{
                          backgroundImage: `url(${post.watcher_avatar})`,
                        }}
                      />
                    </div>
      ...
      <button
        onClick={toggle_button}
        className="fixed bottom-6 right-6 md:hidden p-4 bg-linear-to-r from-blue-500 to-purple-500 text-white rounded-full shadow-lg hover:shadow-xl transition-shadow"
      >
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div
                    className={`h-10 w-10 rounded-full bg-linear-to-r ${user.color}`}
                  />
                  <div>
